...
export const markdownComponents: Components = {
  ...
  blockquote({ children }) {
    return <blockquote className="my-4 pl-4 border-l-4 italic">{children}</blockquote>;
  },

  table({ children }) {
    return (
      <div className="my-4 overflow-x-auto">
        <table className="border min-w-full border-collapse">{children}</table>
      </div>
    );
  },

  th({ children }) {
    return <th className="px-4 py-2 border font-semibold text-left">{children}</th>;
  },
